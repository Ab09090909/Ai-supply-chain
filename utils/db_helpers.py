    ...
    try:
        ...
        if filters:
            for key, value in filters.items():
                ...
                elif isinstance(value, bool): query = query.eq(key, value)
...
def send_notification(recipient_id, title, message, notif_type="info", order_id=None):
    try:
        payload = {"recipient_id": str(recipient_id), "title": title, "message": message, "type": notif_type, "is_read": False}
...
def reduce_product_stock(product_id, qty_sold):
    try:
        ...
        if new_qty <= 0: update_payload["is_available"] = False
