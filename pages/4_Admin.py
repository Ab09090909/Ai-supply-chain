...
try:
    ...
    from utils.constants import REGIONS, SECTORS, ROLE_ICONS
...
# ─────────────────────────────────────────────
if "initialized_admin" not in st.session_state:
    st.session_state.initialized_admin = True
...
@media (max-width: 768px) {
    ...
    .admin-header { flex-direction: column !important; padding: 16px !important; text-align: center; }
...
.admin-header h1 { margin: 0; font-size: 26px; font-weight: 700; color: #f1f5f9; letter-spacing: -0.3px; }
.admin-header p { margin: 4px 0 0; font-size: 13px; color: #64748b; }
.admin-badge {
    margin-left: auto;
    background: #1e3a5f;
...
.kpi-value { font-size: 32px; font-weight: 700; color: #f1f5f9; font-family: 'JetBrains Mono', monospace; line-height: 1; }
.kpi-delta { font-size: 12px; color: #22c55e; margin-top: 6px; }
...
.record-name { font-size: 15px; font-weight: 600; color: #f1f5f9; margin: 0 0 4px; }
.record-meta { font-size: 12px; color: #64748b; margin: 0; }
...
if _apic:
    ...
    name_initial = profile.get("full_name", "A")[0].upper()
    _apic_html = f'<div style="width:80px;height:80px;border-radius:50%;border:3px solid #60a5fa;background:#1e2a3a;display:flex;align-items:center;justify-content:center;font-size:32px;color:#f1f5f9;font-weight:700;">{name_initial}</div>'
...
# Profile edit button
if st.button("✏️ Edit Profile", key="admin_header_edit_btn"):
...
with st.sidebar:
    ...
    st.markdown('<div class="pill pill-success">● API Connected</div>', unsafe_allow_html=True)
    st.markdown('<div class="pill pill-info">● Storage Active</div>', unsafe_allow_html=True)
    
    st.divider()
    st.caption(f"👤 {profile.get('full_name', 'Admin')}")
...
def fetch_overview_stats():
    """Fetch overview statistics with caching."""
    try:
        total_users = supabase.table("profiles").select("id", count="exact").execute().count or 0
        total_producers = supabase.table("profiles").select("id", count="exact").eq("role", "producer").execute().count or 0
        total_merchants = supabase.table("profiles").select("id", count="exact").eq("role", "merchant").execute().count or 0
        total_customers = supabase.table("profiles").select("id", count="exact").eq("role", "customer").execute().count or 0
        ...
        pending_verify = supabase.table("profiles").select("id", count="exact").eq("is_verified", False).execute().count or 0
        ...
        total_revenue = sum(float(r.get("total_price_birr", 0)) for r in revenue_data)
        
        # Active listings
        active_listings = supabase.table("products").select("id", count="exact").eq("is_available", True).execute().count or 0
...
with tab_overview:
    try:
        ...
        with k4:
            st.markdown(f'<div class="kpi-card"><div class="kpi-label">Revenue (Birr)</div><div class="kpi-value">{stats["total_revenue"]:,.0f}</div></div>', unsafe_allow_html=True)
        ...
        with k8:
            st.markdown(f'<div class="kpi-card"><div class="kpi-label">Active Listings</div><div class="kpi-value">{stats["active_listings"]}</div></div>', unsafe_allow_html=True)
...
with tab_users:
    try:
        ...
        logger.error(f"Users tab error: {e}")
...
with tab_verify:
    try:
        ...
        logger.error(f"Verify tab error: {e}")
        st.error(f"⚠️ Error loading verification queue: {str(e)}")

# ══════════════════════════════════════════════
# TAB — DOCUMENTS (Truncated for brevity - same as original with error handling)
# ══════════════════════════════════════════════
with tab_docs:
    try:
        ...
        st.error(f"⚠️ Error loading documents: {str(e)}")

# ══════════════════════════════════════════════
# TAB — DATABASE (Truncated)
# ══════════════════════════════════════════════
with tab_db:
    try:
        ...
        for p in filtered_prods:
            ...
            with st.container(border=True):
                ...
                with c1:
                    ...
                    if img_b64:
                        try:
                            st.image(f"data:image/jpeg;base64,{img_b64}", width=100)
                        except Exception:
                            st.caption("📷 No img")
                    else:
                        st.caption("📷 No img")
                with c2:
                    ...
                    st.caption(f"Sector: {p.get('sector', '—')}  ·  Stock: {p.get('quantity', '—')} {p.get('unit', '')}")
                with c3:
        ...
        for o in filtered_orders:
            ...
            with st.container(border=True):
                ...
                with c1:
                    ...
                    st.caption(f"Date: {o.get('created_at', '')[:10]}  ·  Qty: {o.get('quantity_ordered', '—')}")
                with c2:
    ...
    try:
        ...
        with f1:
            ...
            for o in high_risk_orders[:5]:
                risk_class = "fraud-high" if o.get("total_price_birr", 0) > 100000 else "fraud-med"
...
with tab_activity:
    try:
        ...
        st.error(f"⚠️ Error loading activity: {str(e)}")
...
with tab_settings:
    try:
        ...
        st.error(f"⚠️ Error loading settings: {str(e)}")
...
if st.session_state.get("show_profile_editor"):
    try:
        ...
        logger.error(f"Profile editor error: {e}")
