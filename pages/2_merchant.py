...
if _mpic:
    ...
    _mpic_html = f'<div style="width:80px;height:80px;border-radius:50%;border:3px solid #2563eb;background:#1e2a3a;display:flex;align-items:center;justify-content:center;font-size:32px;color:#f1f5f9;font-weight:700;">{name_initial}</div>'
...
# Profile edit button
if st.button("✏️ Edit Profile", key="merchant_header_edit_btn"):
...
with tab_ai_insights:
    try:
                ...
                with st.spinner("Analyzing your preferences..."):
                    try:
                        ...
                        recs = get_recommendations(
                            commodity=pref_products[0] if pref_products else None,
                            region=pref_regions[0] if pref_regions else None,
                            top_k=5
                        )
                        ...
                        if recs:
                            ...
                            st.info("No recommendations available. Try updating your preferences.")
                    except Exception as e:
        ...
        with mfc_col1:
            mfc_product = st.selectbox("Product", _prod_options, key="mfc_product")
        with mfc_col2:
