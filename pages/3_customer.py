...
try:
    ...
    from utils.shared_ui import (
        render_browse_tab, render_notifications_tab, 
        render_profile_editor_modal, render_fraud_badge
    )
...
# ─────────────────────────────────────────────
if "initialized_customer" not in st.session_state:
    st.session_state.initialized_customer = True
...
if _cpic:
    ...
    name_initial = profile.get("full_name", "C")[0].upper()
    _cpic_html = f'<div style="width:80px;height:80px;border-radius:50%;border:3px solid #7c3aed;background:#1e2a3a;display:flex;align-items:center;justify-content:center;font-size:32px;color:#f1f5f9;font-weight:700;">{name_initial}</div>'
...
# Profile edit button
if st.button("✏️ Edit Profile", key="customer_header_edit_btn"):
                    ...
                    with c3:
                        if status == "pending":
                            ...
                            try:
                                ...
                                ppu_c = float(prod.get("price_birr") or (tot_c / qty_c if qty_c else 0))
                                
                                pdf_cust = generate_agreement_pdf(
                                    producer_name=prod.get("producer_name", "Producer"),
                                    producer_phone="",
...
with tab_wishlist:
    try:
        st.markdown('<div class="section-title">❤️ Saved Products</div>', unsafe_allow_html=True)
        ...
        if not wishlist:
            ...
            <div style="text-align:center;padding:40px;color:#475569;">
                ...
                <div style="font-size:15px;">Your wishlist is empty.</div>
                <div style="font-size:13px;margin-top:6px;">Browse products and save the ones you like.</div>
            </div>
            """, unsafe_allow_html=True)
        else:
...
with tab_ai_insights:
    try:
            ...
            if st.button("Get Recommendations", use_container_width=True, key="ai_rec_btn"):
                with st.spinner("Analyzing market trends..."):
                    try:
                        ...
                        if recs:
                            st.success(f"✅ Found {len(recs)} recommended products!")
                            for r in recs[:3]:
                                st.markdown(f"- **{r.get('commodity', 'Unknown')}** - ETB {r.get('price_etb', 0):,.2f} (⭐ {r.get('avg_rating', 0):.1f})")
                        else:
                            st.info("No recommendations available. Browse products to get personalized suggestions.")
                    except Exception as e:
                        ...
                        if recent_orders:
                            ...
                            for order in recent_orders:
                                ...
                                risk = check_fraud_risk(
                                    sector=prod.get("sector", "Agriculture"),
            ...
            if st.button("Analyze Prices", use_container_width=True, key="ai_price_btn"):
                with st.spinner("Computing market analysis..."):
                    try:
                        ...
                        if recommended > 0:
                            st.success(f"✅ Recommended price for {product}: ETB {recommended:.2f}")
                            st.caption(f"Season: {price_result.get('season', 'N/A')} · Confidence: {price_result.get('confidence', 0):.0%}")
                        else:
                            st.info("Price analysis completed. Check the forecast tab for more details.")
                    except Exception as e:
        ...
        try:
            ...
            if order_count > 5:
                score += 10
            if len(st.session_state.get("wishlist", [])) > 0:
                score += 10
            if verif_status.get("is_verified", False):
                score += 10
            score = min(score, 100)
        ...
        st.caption("💡 Place orders and use AI features to improve your score!")
        
    except Exception as e:
        logger.error(f"AI Insights error: {e}")
        st.error(f"⚠️ Error loading AI insights: {str(e)}")
